...
import cn.petstore.dao.ProductDao;
import cn.petstore.exception.AddProductException;
import cn.petstore.exception.FindProductByIdException;
import cn.petstore.exception.ListProductException;
public class ProductService {
	private ProductDao dao = new ProductDao();
	// 添加商品
	public void addProduct(Product p,int uid) throws AddProductException {
		try {
			dao.addProduct(p,uid);
	...
	//后台系统，根据id删除商品信息
	public void deleteProduct(String id,int uid, String name) {
		try {
			dao.deleteProduct(id,uid,name);
		} catch (SQLException e) {
