...
public class ProductDao {
	// 添加商品
	public void addProduct(Product p,int uid) throws SQLException {
		...
		String sql0="insert a product named"+p.getName();
		String sql2="insert into log(uid,content) values(?,?)";
		runner2.update(sql2, uid,sql0);
	}
	// 查找所有商品
...
//		Object obj = new Object[] { (currentPage - 1) * currentCount, currentCount };
	...
	//后台系统，根据id删除商品信息
	public void deleteProduct(String id, int uid, String name) throws SQLException {
		String sql = "DELETE FROM products WHERE id = ?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		runner.update(sql, id);
		
//		String sql3="select products.name from products where id="+id+"";
//		QueryRunner runner3 = new QueryRunner(DataSourceUtils.getDataSource());
//		String name=(String)runner3.query(sql3, new ScalarHandler());
		
		QueryRunner runner2 = new QueryRunner(DataSourceUtils.getDataSource());
		String sql0="删除了一个名为"+name+"的商品";
		String sql2="insert into log(uid,content) values(?,?)";
		runner2.update(sql2, uid,sql0);
	}
}
